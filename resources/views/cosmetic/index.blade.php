...
@section('content')
    ...
    <h2>Data Produk
        <a class="btn btn-danger btn-sm float-end" href="{{ url('cosmetic/create') }}">Tambah Data Produk</a>
    </h2>
            ...
            @foreach ($rows as $row)
                <tr class="table-light">
                    ...
                    <td class="text-center">
                        <a class="btn btn-primary" href="{{ url('cosmetic/' . $row->produk_id . '/edit') }}">Edit</a>
                    </td>
                    <td class="text-center">
                        <form class=" btn-sm float-none" action="{{ url('cosmetic/' . $row->produk_id) }}" method="POST">
                            @method('DELETE')
                            @csrf
